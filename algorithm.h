...
namespace algorithm
{
    ...
    template <typename T>
    void merge(std::vector<T> *collection, std::size_t start, std::size_t mid, std::size_t end, bool ASC);

    template <typename T>
    void mergeSort(std::vector<T> *collection, const std::size_t start, const std::size_t end, bool ASC = true)
    {
        if (start >= end) return;
        std::size_t mid = (start + end) / 2;
        mergeSort(collection, start, mid, ASC);
        mergeSort(collection, mid + 1, end, ASC);

        merge(collection, start, mid, end, ASC);
    }

    template <typename T>
    void merge(std::vector<T> *collection, const std::size_t start, const std::size_t mid, const std::size_t end, bool ASC)
    {
